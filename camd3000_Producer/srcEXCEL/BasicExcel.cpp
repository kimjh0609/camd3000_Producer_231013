...
ULONG Workbook::Read(const char* data, size_t dataSize)//pjs20231011
{
	ULONG bytesRead = 0;
...
//				break;
			...
			default:
			{
				if ((bytesRead+100) > dataSize)//88831
				{
					if (bytesRead == 188988)
						bytesRead++;
				}
				if (bytesRead < dataSize) {//pjs20231011
					Record rec;
	...
	while(code == CODE::BOF)
	{
		...
		switch(bof.type_)
		{
			case WORKBOOK_GLOBALS:
				bytesRead += workbook_.Read(data+bytesRead, dataSize);//pjs20231011
				break;
